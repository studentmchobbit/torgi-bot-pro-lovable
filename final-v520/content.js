(() => {
    ...
    function hasBuyButtonSpinner(el) {
        ...
        return !!(
            el.querySelector?.(".spinner, [class*='spinner'], app-icon-status-in-progress, .rotate, [class*='progress'], [class*='loading'], #initial-progress-indicator, [id*='initial-progress']") ||
    ...
    function schedulePreStartSessionRefresh(timing) {
        ...
        const startMs = timing.start.getTime();
        const stopBeforeMs = Number(settings?.preStartSessionRefreshStopBefore ?? 30000);  // было 45000, сдвинули ближе к старту
        const intervalMs = Number(settings?.preStartSessionRefreshInterval ?? 60000);
        const msUntilStop = startMs - stopBeforeMs - nowMs;

        // v5.5.2: не начинаем refresh пока до старта больше чем intervalMs + stopBeforeMs
        // Это убирает лишние refresh за 143сек до старта
        if (msUntilStop <= intervalMs) {
    ...
    function startCombatPoll() {
        ...
        // v5.5.2: запускаем MutationObserver сразу, не ждём появления кнопки
        startObserverLoop(true);

        const fastInterval = 40;
        const targetStartMs = lastTimingInfo?.start?.getTime?.() || 0;

        // v5.5.2: сохраняем startMs в storage для background combat engine
        if (targetStartMs) {
            try {
                SESSION_STORAGE.set({ torgiBotCombatTiming: { startMs: targetStartMs } }).catch(() => {});
            } catch (_) {}
        }

        // v5.5.2: запускаем combat engine в background
        const lotId = getLotIdFromUrl();
        if (lotId && targetStartMs) {
            safeSendMessage({ type: "START_COMBAT_ENGINE", lotId }).catch(() => {});
        }

        const getApiPollMs = nowMs => {
            if (!targetStartMs) return Number(settings?.apiWatchdogInterval ?? 500);
            if (nowMs < targetStartMs - 2000) return 500;  // было 1000
        ...
        combatPollTimer = setInterval(() => {
            ...
            if (isBuyButtonPresentDom()) {
                ...
                clearInterval(combatPollTimer);
                combatPollTimer = null;
                log("Боевой режим: кнопка «Купить» обнаружена в", new Date().toISOString());
                runLogic();
                return;
            }
    ...
    try {
        chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
            (async () => {
                ...
                if (message?.type === "COMBAT_RELOAD_IMMINENT") {
                    pageReadyAt = Date.now();
                    log("COMBAT_RELOAD_IMMINENT: pageReadyAt сброшен");
                    sendResponse({ ok: true });
                    return;
                }
            })().catch(error => {
